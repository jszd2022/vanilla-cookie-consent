...
<aside id="cookies-policy" class="cookies cookies--no-js"
       data-text="<?= json_encode(lcc_trans('cookieConsent::cookies.details')) ?>">
    <div class="cookies__alert">
        <div class="cookies__container">
            <div class="cookies__wrapper">
                <h2 class="cookies__title">
                    <?= lcc_trans('cookieConsent::cookies.title') ?>
                </h2>
                <div class="cookies__intro">
                    <p><?= lcc_trans('cookieConsent::cookies.intro') ?></p>
                    <?php if ($policy): ?>
                        <p><?= lcc_trans('cookieConsent::cookies.link', ['url' => $policy]) ?></p>
                    <?php endif; ?>
                </div>
                <div class="cookies__actions">
                    <?= Cookies::renderButton(action: 'accept.essentials', label: lcc_trans('cookieConsent::cookies.essentials'), attributes: ['class' => 'cookiesBtn cookiesBtn--essentials']) ?>
                    <?= Cookies::renderButton(action: 'accept.all', label: lcc_trans('cookieConsent::cookies.all'), attributes: ['class' => 'cookiesBtn cookiesBtn--accept']) ?>
                </div>
            </div>
        </div>
        <a href="#cookies-policy-customize" class="cookies__btn cookies__btn--customize">
            <span><?= lcc_trans('cookieConsent::cookies.customize') ?></span>
        ...
        <div class="cookies__expandable cookies__expandable--custom" id="cookies-policy-customize">
            <form action="<?= lcc_route('cookieconsent.accept.configuration') ?>" method="post"
                    ...
                    <?php foreach ($cookies->getCategories() as $category): ?>
                        <div class="cookies__section">
                            ...
                            <a href="#cookies-policy-<?= $category->key() ?>"
                               class="cookies__details"><?= lcc_trans('cookieConsent::cookies.details.more') ?></a>
                        </div>
                    <?php endforeach; ?>
                </div>
                <div class="cookies__save">
                    <button type="submit"
                            class="cookiesBtn__link"><?= lcc_trans('cookieConsent::cookies.save') ?></button>
                </div>
            </form>
        </div>
    </div>
